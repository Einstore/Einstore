...
export async function iosInstallRoutes(app) {
    app.post("/builds/:id/ios/install-link", { preHandler: requireTeam }, async (request, reply) => {
        ...
        const manifestToken = generateInstallToken({ buildId, teamId, userId, action: "manifest" }, INSTALL_TTL_SECONDS);
        const downloadToken = generateInstallToken({ buildId, teamId, userId, action: "download" }, INSTALL_TTL_SECONDS);
        const installToken = generateInstallToken({ buildId, teamId, userId, action: "install" }, INSTALL_TTL_SECONDS);
        const baseUrl = resolveBaseUrl(request);
        const manifestUrl = `${baseUrl}/builds/${buildId}/ios/manifest?token=${manifestToken}`;
        const itmsServicesUrl = `itms-services://?action=download-manifest&url=${encodeURIComponent(manifestUrl)}`;
