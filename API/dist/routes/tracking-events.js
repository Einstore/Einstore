...
const ensureServices = (meta) => {
    const fromList = meta.services ?? [];
    const inferred = [];
    if (meta.analytics)
        inferred.push("analytics");
    if (meta.errors)
        inferred.push("errors");
    if (meta.distribution)
        inferred.push("distribution");
    if (meta.device)
        inferred.push("devices");
    if (meta.usage)
        inferred.push("usage");
    if (meta.crash)
        inferred.push("crashes");
    const unique = new Set([...fromList, ...inferred]);
    return Array.from(unique).filter((value) => value === "analytics" ||
        value === "errors" ||
        value === "distribution" ||
        value === "devices" ||
        value === "usage" ||
...
const buildTrackingEvents = (buildId, teamId, userId, parsedBody, meta, services) => {
    const base = {
        buildId,
        teamId,
        userId,
...
export async function trackingEventRoutes(app) {
    const handleCreateEvents = async (request, reply, buildIdFromParams) => {
        ...
        const events = buildTrackingEvents(buildId, teamId, request.auth?.user.id, parsed.data, meta, services);
        const created = await prisma.$transaction(events.map((data) => prisma.trackingEvent.create({ data })));
