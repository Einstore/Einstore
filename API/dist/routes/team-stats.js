...
export async function teamStatsRoutes(app) {
    app.get("/teams/stats", { preHandler: requireTeam }, async (request, reply) => {
        ...
        const downloadsThisMonthCount = downloadsByBuild.reduce((sum, row) => sum + row._count._all, 0);
        ...
        const uploadsInBillingCycleBytes = buildsInBillingCycle.reduce((sum, build) => sum + build.sizeBytes, 0);
        const uploadsInBillingCycleAppCount = new Set(buildsInBillingCycle.map((build) => build.version.appId)).size;
        return reply.send({
