...
const saveLocal = async (teamId, buffer) => {
    await ensureDir();
    const filePath = path.join(LOGO_DIR, `${teamId}.png`);
    await fs.promises.writeFile(filePath, buffer);
    return filePath;
};
const uploadToSpaces = async (teamId, buffer) => {
...
export async function teamLogoRoutes(app) {
    ...
    app.get("/teams/:id/logo", async (request, reply) => {
        ...
        if (bucket && client) {
            ...
            try {
                const signed = await getSignedUrl(client, new GetObjectCommand({ Bucket: bucket, Key: key }), { expiresIn: 60 });
